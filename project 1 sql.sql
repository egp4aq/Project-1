# SQL query to find average fuel price
# SELECT AVG(Fuel_Price) FROM walmart56.walmar


# use walmart56 schema to create another table with data from a local csv file with holiday information for uber
...
CREATE TABLE `holiday12` (
  `pickup_dt` varchar(50),
  `borough` varchar(50),
  `pickups` varchar(50) DEFAULT NULL,
  `spd` varchar(50) DEFAULT NULL,
  `vsb` varchar(15) DEFAULT NULL,
  `temp` varchar(50) DEFAULT NULL,
  `dewp` varchar(50) DEFAULT NULL,
  `slp` varchar(50) DEFAULT NULL,
  `pcp01` varchar(50) DEFAULT NULL,
  `pcp06` varchar(50) DEFAULT NULL,
  `pcp24` varchar(50) DEFAULT NULL,
  `sd` varchar(50) DEFAULT NULL,
  `hday` varchar(50) DEFAULT NULL,

  KEY `pickup_dt` (`pickup_dt`),
  KEY `borough` (`borough`),
  KEY `pickups` (`pickups`),
  KEY `spd` (`spd`),
  KEY `vsb` (`vsb`),
...
# loading in the local file to populate the holiday table we just created   
#LOAD DATA LOCAL INFILE "C:\Users\ds2002admin\Downloads\uber_nyc_enriched.csv" INTO TABLE holiday
...
# We want to add another table here from some other data source. 